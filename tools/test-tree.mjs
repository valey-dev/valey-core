...
UI.bagKey('ArrowDown');
check('вниз с последнего — остаётся на последнем', UI.treeSelected() === 'door', UI.treeSelected());
...
check('в подробном виде вниз останавливается на последнем навыке', UI.treeSelected() === 'task', UI.treeSelected());
UI.bagKey('ArrowDown');
check('повторная стрелка вниз не возвращает наверх', UI.treeSelected() === 'task', UI.treeSelected());
