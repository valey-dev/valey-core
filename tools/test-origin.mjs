...
// ------------------------------------------------ whose files: without a server
{
  ...
  ok('the transcript file knows all its agents', JSON.stringify(fileOwners('/p/a.md', snap)) === '["a","c"]', fileOwners('/p/a.md', snap));
  ok('artifact too', JSON.stringify(fileOwners('/p/b.png', snap)) === '["b"]', fileOwners('/p/b.png', snap));
  ok('nobody else\'s way', fileOwners('/etc/hosts', snap).length === 0, fileOwners('/etc/hosts', snap));
}
...
const alive = async (name) => {
  ...
  ok(`${name} - office is alive`, r.status === 200 && r.j && r.j.owner === true, r.status);
};

try {
  // -------------------------------------------------------------- rebinding
  const bound = await req('/', { headers: { host: 'evil.example' } });
  ok('someone else’s name is on a loop - they don’t give the page, and they answer like a closed office', bound.status === 404, bound.status);
  const boundApi = await req('/api/state', { headers: { host: `evil.example:${PORT}` } });
  ok('and they don’t give away a photo under someone else’s name', boundApi.status === 404, boundApi.status);
  for (const h of [`localhost:${PORT}`, `127.0.0.1:${PORT}`, `[::1]:${PORT}`, 'localhost']) {
    const own = await req('/api/state', { headers: { host: h } });
    ok(`your name (${h}) - as it was`, own.status === 200, own.status);
  }

  // ---------------------------------------------------------- a foreign tab
  const foreign = await json('/api/settings', { lang: 'en' }, { origin: 'http://evil.example' });
  ok('POST from someone else\'s Origin - refusal', foreign.status === 403 && foreign.j.errorKey === 'err.crossSite', foreign);
  const sfs = await json('/api/settings', { lang: 'en' }, { 'sec-fetch-site': 'cross-site' });
  ok('and by Sec-Fetch-Site: cross-site - too, even without Origin', sfs.status === 403, sfs.status);
  const nul = await json('/api/task', { agentId: 'x', text: 'y' }, { origin: 'null' });
  ok('Origin: null - this is not your own page', nul.status === 403, nul.status);
  const own = await json('/api/settings', { lang: 'ru' }, { origin: `http://127.0.0.1:${PORT}` });
  ok('your Origin - settings are saved', own.status === 200 && own.j.ok === true, own);
  const ownName = await json('/api/settings', { lang: 'ru' }, { origin: `http://localhost:${PORT}`, host: `localhost:${PORT}` });
  ok('your Origin named localhost - too', ownName.status === 200, ownName.status);
  const noOrigin = await json('/api/settings', { lang: 'ru' });
  ok('without Origin at all (curl, tests) - we believe', noOrigin.status === 200, noOrigin.status);
  const foreignGet = await req('/api/state', { headers: { origin: 'http://evil.example' } });
  ok('GET from someone else’s Origin they answer: someone else’s page won’t be able to read it anyway', foreignGet.status === 200, foreignGet.status);
  const beacon = await json('/api/gone', { id: 'nobody' }, { 'sec-fetch-site': 'same-origin' });
  ok('the closed tab beacon passes', beacon.status === 200, beacon.status);
  const foreignBeacon = await json('/api/gone', { id: 'nobody' }, { 'sec-fetch-site': 'cross-site' });
  ok('but someone else’s beacon is not', foreignBeacon.status === 403, foreignBeacon.status);

  // -------------------------------------------------------------- the body type
  const plain = await req('/api/settings', { method: 'POST', headers: { 'content-type': 'text/plain' }, body: '{"lang":"en"}' });
  ok('JSON under the guise of text/plain - not accepted', plain.status === 415 && plain.j.errorKey === 'err.notJson', plain);
  const bare = await req('/api/enter', { method: 'POST', body: '{"code":"x"}' });
  ok('and without a title at all - too, even in an open pen', bare.status === 415, bare.status);
  const charset = await req('/api/settings', { method: 'POST', headers: { 'content-type': 'application/json; charset=utf-8' }, body: '{"lang":"ru"}' });
  ok('application/json with charset is still JSON', charset.status === 200, charset.status);
  ...
  ok('broken JSON in /api/enter - 400, not death', broken.status === 400 && broken.j.errorKey === 'err.badJson', broken);
  ...
  ok('array instead of object - 400', arr.status === 400, arr.status);
  ...
  ok('empty body - empty object, as before', empty.status === 200, empty.status);
  const huge = await req('/api/task', {
    method: 'POST', headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ agentId: 'x', text: 'я'.repeat(100 * 1024) }),
  });
  ok('body without end - 413, not memory to end', huge.status === 413 && huge.j.errorKey === 'err.tooBig', huge);
  ...
  ok('someone else\'s method in the login handle - not 500', method.status !== 500, method.status);
  ...
  if (mods.some((m) => m.id === 'radio')) {
    ...
    ok('the module section is in the settings from the first request, before any saving',
      boot.status === 200 && boot.j && boot.j.settings && 'spotify' in boot.j.settings, boot.j && Object.keys(boot.j.settings || {}));
  } else {
    ok('There are no modules in the assembly - we don’t ask about their settings', true, mods.length);
  }

  // --------------------------------------------------------------- the files
  const file = await req('/api/file?path=' + encodeURIComponent('/etc/hosts'));
  ok('file not from the transcript will not be given', file.status === 403, file.status);
} catch (e) {
  bad += 1;
