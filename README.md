...
`WASD` walk · `SHIFT` run · `SPACE` talk, and drink at the cooler · `TAB` the round · `N` notes · `C` change your look · `P` window on the world · `U` office colour · `M` sound · `R` radio · `H` the pager you put off · `+` `0` scale · `ESC` back. The security room is below the floor and has the cameras.
