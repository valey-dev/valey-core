...
const DEFAULTS = {
  ...
  lang: 'ru',
  // Пак имён: 'auto' — идти за языком офиса, иначе id пака ('ru', 'en').
  // Выбранный руками переживает переключение интерфейса — это и значит
  // «имена откреплены от языка».
  namePack: 'auto',
  // Каким паком выданы имена, которые лежат в names. Не настройка, а отметка:
  // без неё сервер не отличит «пак не трогали» от «пак сменили, пока офис не
  // работал», и офис бы либо не переименовался вовсе, либо переименовывался
  // на каждом снимке.
  namesPack: '',
...
export async function patchSettings(patch) {
  ...
  cache = {
    ...
    // Отметка пака идёт рядом с именами и той же заменой: она их описывает.
    // Пустая строка тут — законное значение («имена ещё никем не выданы»),
    // поэтому || не годится.
